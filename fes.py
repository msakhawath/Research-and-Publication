...
unique_features = data['Number of Features'].unique()
...
# Calculate correlations for each number of features
for feature_count in sorted(unique_features):
    feature_subset = data[data['Number of Features'] == feature_count]
    corr_values = calculate_correlations(feature_subset)
...
for label, values in correlations.items():
    plt.plot(sorted(unique_features), values, label=label, marker='o')
plt.xticks(rotation=45)
plt.title('Spearman Correlation vs Number of Features')
