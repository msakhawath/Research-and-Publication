...
def adjust_test_set(X_train, X_test, y_test, anomaly_percentage=0.1):
    normal_data = X_train
...
for filename in os.listdir(folder_path):
    if filename.endswith(file_extension):
        ...
        X_train = remove_nan_values(data['x'])
        X_test = remove_nan_values(data['tx'])
        y_test = data['ty'][~np.isnan(data['ty'])]

        initial_anomaly_percentage = np.mean(y_test) * 100
        X_train_adjusted, X_test_adjusted, y_test_adjusted = adjust_test_set(X_train, X_test, y_test, anomaly_percentage=0.1)

        if X_train_adjusted is None:
            skipped_datasets.append(filename)
            continue
...
csv_file_path = os.path.join(folder_path, 'adjusted_results_all.csv')
results_df.to_csv(csv_file_path, index=False)

print(f'Adjusted results saved to {csv_file_path}')
if skipped_datasets:
    print("Skipped datasets due to issues (e.g., insufficient normal samples, NaN values):", ', '.join(skipped_datasets))
