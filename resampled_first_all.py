...
def resample_test_set(X_test, y_test, anomaly_percentage):
    ...
    if n_anomalies > len(anomaly_data):
        return None, None  # Not enough anomaly samples
...
for filename in os.listdir(folder_path):
    if filename.endswith(file_extension):
        ...
        # Anomaly percentages for resampling
        anomaly_percentages = [0.1, 0.2, 0.3, 0.4]

        # Standardize the training data
        X_train_scaled = standardizer(X_train)

        for anomaly_percentage in anomaly_percentages:
            # Resample test set
            X_test_resampled, y_test_resampled = resample_test_set(X_test, y_test, anomaly_percentage)

            # Skip this dataset if not enough anomaly samples
            if X_test_resampled is None:
                continue

            resampled_anomaly_percentage = np.mean(y_test_resampled) * 100

            # Standardize the resampled test data
            _, X_test_scaled = standardizer(X_train, X_test_resampled)

            # Set the contamination rate to the current anomaly percentage
            fixed_contamination = anomaly_percentage
            ...
            for clf_name, clf in classifiers.items():
                ...
                y_scores = clf.decision_function(X_test_scaled) if hasattr(clf,
                                                                           'decision_function') else clf.predict_proba(
                    X_test_scaled)[:, 1]
                ...
                result = {
                    ...
                    'ROC AUC': safe_roc_auc_score(y_test_resampled, y_scores),
                    ...
                    'Resampling Percentage': anomaly_percentage * 100
                }
...
csv_file_path = os.path.join(folder_path, 'test.csv')
